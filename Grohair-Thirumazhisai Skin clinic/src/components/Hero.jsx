...
export default function Hero() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('https://grohairgloskinthirumazhisai.com/api/email.php', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        setStatus('idle')
        // Redirect to thankyou page for tracking and clear feedback
        navigate('/thankyou')
      } else {
        setStatus('error')
      }
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 pt-10 pb-6 flex-1 justify-center">
        ...
        <div className="mb-4 text-center pt-4">
          <h1 className="text-[26px] leading-[32px] font-bold text-white mb-1 drop-shadow-md">
            ...
            Doctor in Thirumazhisai
          </h1>
          <p className="text-[14px] text-white/90 drop-shadow-md pb-1">
            Best Skin Clinic in Thirumazhisai
          </p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col gap-2 mt-1">
            ...
            <button
              ...
              className="btn-shimmer w-full text-white text-[15px] font-bold py-3.5 rounded-[12px] shadow-[0_4px_15px_rgba(139,0,0,0.3)] mt-2 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 active:scale-95 transition-transform"
            >
              {status === 'sending' ? (
                <>
